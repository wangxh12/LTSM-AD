...
from .backbone import PointDualAttentionReconstructionModel


def _required_int(config: dict[str, Any], key: str) -> int:
    if key not in config:
        raise KeyError(f"Expected model.{key} in model_dual_attn config")
...
def build_model_config(config: dict[str, Any]) -> dict[str, Any]:
    if "data" in config and "model" in config:
        ...
        return {
            "model_type": "model_dual_attn",
            "model_id": config.get("model_id", "dual-attn"),
...
class Model(nn.Module, ModelHubMixin):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.model = PointDualAttentionReconstructionModel(**model_kwargs)
