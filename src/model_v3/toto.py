...
from .utils import make_batched_block_mask
...
class RMSNorm(torch.nn.Module):
    """RMSNorm implementation without xFormers dependency."""
...
class Transformer(torch.nn.Module):
    ...
    def __init__(
        ...
        assert d_model % n_heads == 0, "Embedding dimension must be divisible by number of heads."
    ...
    def forward(
        self,
        inputs: Float[torch.Tensor, "batch variate seq_len embed_dim"],
        id_mask: Float[torch.Tensor, "batch #variate seq_len"] | None = None,
        # kv_cache: Optional[KVCache] = None,
        # variate_label_embeds: Optional[Float[torch.Tensor, "batch variate 1 embed_dim"]] = None,
    ) -> Float[torch.Tensor, "batch variate seq_len embed_dim"]:

        if inputs.ndim != 4:
            raise ValueError(f"inputs must have shape [batch, variate, seq_len, embed_dim], got {tuple(inputs.shape)}")
        batch, variate, seq_len, _ = inputs.shape
        ...
        spacewise_attention_mask = None
        if id_mask is not None:
            if tuple(id_mask.shape) != (batch, variate, seq_len):
                raise ValueError("id_mask must match inputs shape [batch, variate, seq_len]")
