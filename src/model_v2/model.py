...
def _required_int(config: dict[str, Any], key: str) -> int:
    if key not in config:
        raise KeyError(f"Expected model.{key} in model_v2 config")
...
def _required_heads(config: dict[str, Any]) -> int:
    ...
    raise KeyError("Expected model.num_heads or model.n_heads in model_v2 config")
...
def _required_ffn(config: dict[str, Any]) -> int:
    ...
    raise KeyError("Expected model.d_ff or model.d_ffn in model_v2 config")
...
def _target_fields(config: dict[str, Any]) -> list[str]:
    ...
    if not fields:
        raise KeyError("Expected non-empty data.target_fields or top-level features in model_v2 config")
    return list(fields)
...
def build_model_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "model_type": "model_v2",
        "model_id": str(config.get("model_id", "model-v2")),
